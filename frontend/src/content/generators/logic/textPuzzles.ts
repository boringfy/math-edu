...
function numberRuleFor(tier: Tier, rng: Rng): Rule {
  const length = 5;
  const kinds =
    tier === 1
  ...
  switch (rng.pick([...kinds])) {
    case 'add': {
      const start = rng.randInt(1, 9);
      const step = rng.randInt(2, tier === 1 ? 5 : 9);
      return {
    ...
    case 'shrinking': {
      const step = rng.randInt(3, 9);
      const start = step * length + rng.randInt(1, 20);
      return {
        terms: Array.from({ length }, (_, i) => start - i * step),
    ...
    case 'growing': {
      ...
      const start = rng.randInt(1, 6);
      const firstStep = rng.randInt(1, 3);
      const terms = [start];
...
export function numberSequence(tier: Tier, rng: Rng): Question {
  const { terms, explanation } = numberRuleFor(tier, rng);
...
export function letterSequence(tier: Tier, rng: Rng): Question {
  const growing = tier > 1 && rng.next() < 0.5;
  const step = rng.randInt(1, growing ? 2 : 4);
  ...
  const start = rng.randInt(0, LETTERS.length - 1 - span);
...
export function oddWordOut(rng: Rng): Question {
  const [home, away] = rng.shuffle(CATEGORIES).slice(0, 2);
  const family = rng.shuffle(home.words).slice(0, 3);
  const stranger = rng.pick(away.words);
  return makeQuestion(
    rng,
    `Which word does not belong?\n\n${rng.shuffle([...family, stranger]).join(',  ')}`,
    stranger,
    family,
...
export function oddNumberOut(tier: Tier, rng: Rng): Question {
  const base = rng.randInt(3, tier === 3 ? 9 : 5);
  const multiples = rng.shuffle(Array.from({ length: 12 }, (_, i) => (i + 2) * base)).slice(0, 3);
  // An intruder that is close to the others but misses the rule.
  let stranger = rng.pick(multiples) + rng.pick([1, -1, 2, -2]);
  while (stranger % base === 0 || stranger <= 0 || multiples.includes(stranger)) stranger += 1;

  return makeQuestion(
    rng,
    `Which number does not belong?\n\n${rng.shuffle([...multiples, stranger]).join(',  ')}`,
    String(stranger),
...
export function analogy(rng: Rng): Question {
  const relation = rng.pick(RELATIONS);
  const [example, target] = rng.shuffle(relation.pairs).slice(0, 2);
  const others = rng.shuffle(RELATIONS.filter((r) => r !== relation));
  const distractors = [
    ...
    ...others.slice(0, 2).map((r) => rng.pick(r.pairs)[1]),
  ].filter((w) => w !== target[1]);
...
export function syllogism(tier: Tier, rng: Rng): Question {
  const [a, b, c] = rng.shuffle(NONSENSE).slice(0, 3);
...
export function balanceScale(tier: Tier, rng: Rng): Question {
  const [small, middle, big] = rng.shuffle(BALANCE_ITEMS).slice(0, 3);
  const first = rng.randInt(2, tier === 1 ? 3 : 4);
  const second = rng.randInt(2, tier === 3 ? 5 : 3);
 ...
 */
export function logicGrid(tier: Tier, rng: Rng): Question {
  ...
  const size = 4;
  const names = rng.shuffle(PEOPLE).slice(0, size);
  const set = rng.pick(SETS);
  const items = rng.shuffle(set.items).slice(0, size);
  const solution = rng.shuffle(items.map((_, i) => i));

  const clues: Clue[] = [];
  const giveaways = tier === 1 ? 2 : tier === 2 ? 1 : 0;
  for (const subject of rng.shuffle(names.map((_, i) => i)).slice(0, giveaways)) {
  ...
  const asked = rng.randInt(0, size - 1);
  const owner = names[solution.indexOf(asked)];
  const lines = rng.shuffle(needed).map((c) =>
    c.kind === 'is'
