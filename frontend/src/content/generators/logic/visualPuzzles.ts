...
function distinctFrom(correct: Tile, candidates: Tile[], rng: Rng): Tile[] {
  const kept: Tile[] = [];
  ...
  for (let guard = 0; kept.length < 3 && guard < 50; guard++) {
    const tile = randomShapesTile(4, rng);
...
function drawnQuestion(opts: {
  ...
  const tiles = rng.shuffle([opts.correct, ...distinctFrom(opts.correct, opts.distractors, rng)]);
...
export function shapeSeries(tier: Tier, rng: Rng): Question {
  const kind = rng.pick(SHAPE_KINDS);
  const filled = rng.next() < 0.5;

  if (tier === 1 || rng.next() < 0.4) {
    // One more each time.
    ...
    return drawnQuestion({
      ...
      distractors: [
        ...
        shapesTile(rng.pick(SHAPE_KINDS.filter((k) => k !== kind)), 4, filled),
      ],
  ...
  if (tier === 2) {
    ...
    const order = rng.shuffle(SHAPE_KINDS);
...
export function matrixPattern(tier: Tier, rng: Rng): Question {
  const kind = rng.pick(SHAPE_KINDS);
  ...
  const kinds = rng.shuffle(SHAPE_KINDS).slice(0, 3);
  const filled = tier === 3;
...
const figureFor = (tier: Tier, rng: Rng): Tile =>
  tier === 3
    ? asymmetricFigure(4, rng.randInt(5, 7), rng)
    : asymmetricFigure(3, rng.randInt(3, 4), rng);

export function rotation(tier: Tier, rng: Rng): Question {
  const figure = figureFor(tier, rng);
  const turns =
    tier === 1
...
export function mirrorImage(tier: Tier, rng: Rng): Question {
  const figure = figureFor(tier, rng);
  return drawnQuestion({
 ...
 */
export function shapeOddOneOut(tier: Tier, rng: Rng): Question {
  const kinds = rng.shuffle(SHAPE_KINDS);
  const rule = rng.pick(tier === 1 ? ['fill', 'shape'] : ['fill', 'shape', 'count']);
