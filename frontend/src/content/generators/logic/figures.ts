...
import { PuzzleShape, ShapeKind, Tile } from '../../contract';
 ...
 */
export function asymmetricFigure(size: number, filledCells: number, rng: Rng): Tile {
  for (let attempt = 0; attempt < 200; attempt++) {
    const cells = Array<boolean>(size * size).fill(false);
    for (const i of rng.shuffle(cells.map((_, i) => i)).slice(0, filledCells)) cells[i] = true;
...
export const randomShapesTile = (maxCount: number, rng: Rng): Tile =>
  shapesTile(rng.pick(SHAPE_KINDS), rng.randInt(1, maxCount), rng.next() < 0.5);
