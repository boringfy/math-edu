import { CakeCutTask, Question, Tier } from '../types';
import { nextId, pick } from './generator';
...
export function cakeCutQuestion(tier: Tier): Question {
  const task = pick(CAKE_PUZZLES[tier]);
  return {
    id: nextId(),
