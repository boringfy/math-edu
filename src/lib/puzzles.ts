import { Grade, MapStop, ProgressMap, Question, Tier } from '../types';
import { Gen, shuffle } from './generator';
import {
  matrixPattern,
  mirrorImage,
  rotation,
...
/** The kinds of reasoning the logic map trains. */
...
const GENERATORS: Record<PuzzleFamily, (tier: Tier) => Question> = {
 ...
 */
const AVAILABLE: Record<Grade, PuzzleFamily[]> = {
...
/**
 * Deals the set's question count out over the families it names, taking one
 * from each in turn so a set of six across two families is three and three
 * rather than five and one.
 */
export function generatePuzzleSet(set: PuzzleSet): Question[] {
  const families = shuffle(set.focus);
  const gens: Gen[] = Array.from(
    { length: set.questionCount },
    (_, i) => () => GENERATORS[families[i % families.length]](set.tier),
  );
  return shuffle(gens.map((gen) => gen()));
}

export const isSetUnlocked = (set: PuzzleSet, progress: ProgressMap): boolean =>
  isStopUnlocked(PUZZLE_SETS[set.grade], set, progress);

export const currentSet = (grade: Grade, progress: ProgressMap): PuzzleSet =>
  currentStop(PUZZLE_SETS[grade], progress) as PuzzleSet;

export const setStarsEarned = (grade: Grade, progress: ProgressMap): number =>
  starsOnMap(PUZZLE_SETS[grade], progress);
